    ...
    <div class="grid h-full grid-cols-3 gap-3">
        <div class="h-full p-4 rounded bg-gray-50">
            ...
            <div class="flex flex-col gap-4 pt-2 ">
                <div class="">
                    <label class="">Nom du magasin</label>
                    <input type="text" class="w-full p-2 mt-2" placeholder="Nom du magasin" wire:model="nom">
                </div>
                <div class="">
                    <label class="">Adresse</label>
                    <input type="text" class="w-full p-2 mt-2" placeholder="Adresse du magasin" wire:model="adresse">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="">
                        <label class="">Position GPS(L)</label>
                        <input type="text" class="w-full p-2 mt-2" placeholder="Position GPS(L) du magasin" wire:model="longi">
                    </div>
                    <div class="">
                        <label class="">Position GPS(l)</label>
                        <input type="text" class="w-full p-2 mt-2" placeholder="Position GPS(L) du magasin" wire:model="lat">
                    </div>
                </div>
                ...
                <div class="flex " x-data="{select : true}">
                    ...
                    <select name="" id="" class="flex-1 py-2 border" wire:model="ville_id">
                        <option value="">Choix ville</option>
                        @foreach ($villes as $vl)
                            <option value="{{$vl->id}}">{{$vl->nom}}</option>
                        @endforeach
                    </select>
        ...
        <div class="col-span-2 p-4 rounded bg-gray-50">
            ...
            <div class="grid w-full grid-cols-2 gap-4 py-3">
                @forelse ($magasins as $mg)
                    <div class="flex items-center rounded shadow cursor-pointer" wire:click="fillInputs({{$mg}})">
                        ...
                        <div class="flex-1 px-2">
                            <p><b>{{ $mg->nom }}</b></p>
                            <p>Adresse : {{ $mg->adresse }}</p>
                            <p>Ville de {{ $mg->ville }} / Province de {{ $mg->province }}</p>
                        </div>
                    </div>
                @empty
